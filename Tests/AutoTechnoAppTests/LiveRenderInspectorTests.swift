...
struct LiveRenderInspectorTests {
    ...
    func inspectorControlIsReachable() throws {
        ...
        #expect(contentView.contains("private var bottomControlBar: some View"))
        #expect(contentView.contains("Spacer(minLength: 160)"))
        #expect(contentView.contains("HStack(spacing: 16)"))
        #expect(contentView.contains(".padding(.bottom, 22)"))
        #expect(!contentView.contains("Text(engine.positionText)"))
        #expect(contentView.contains(".padding(.bottom, 28)"))
