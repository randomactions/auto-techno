    ...
    private func logRepeatHoldEvolutionPreparation(
        ...
        for evidence in prepared.repeatHoldEvolutionEvidence {
            Self.successorPreparationLogger.info(
                "Hold evolution prepared phrase=\(prepared.plan.phraseIndex + 1, privacy: .public) version=\(evidence.version, privacy: .public) effect=\(evidence.patternFamily.effectKind.rawValue, privacy: .public) family=\(evidence.patternFamily.rawValue, privacy: .public) target=\(evidence.patternFamily.target.rawValue, privacy: .public) qualified=\(evidence.qualified, privacy: .public) failure=\(evidence.conciseFailureCode, privacy: .public) frames=\(evidence.frameCount, privacy: .public) high-band-reduction-db=\(evidence.highBandReductionDB, privacy: .public) loop-captured=\(evidence.looperCapturedFrameCount, privacy: .public) loop-replayed=\(evidence.looperReplayedFrameCount, privacy: .public) loop-expected=\(evidence.looperExpectedReplayedFrameCount, privacy: .public) loop-shortest=\(evidence.looperShortestReplayFrameCount, privacy: .public) loop-boundaries=\(evidence.looperBoundaryFrameCount, privacy: .public)/\(evidence.looperExpectedBoundaryFrameCount, privacy: .public) loop-source-exact=\(evidence.looperSourceReuseExact, privacy: .public) loudness-delta-db=\(evidence.loudnessDeltaDB, privacy: .public) endpoints-exact=\(evidence.endpointsExact, privacy: .public) full-mix-routing-exact=\(evidence.fullMixRoutingExact, privacy: .public) safety=\(evidence.signalSafetyValid, privacy: .public) primary=\(evidence.primarySampleHash, privacy: .public) variant=\(evidence.variantSampleHash, privacy: .public)"
            )
        }
    }
