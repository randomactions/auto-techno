...
public struct MusicalIntentMapper: Sendable {

    /// Produces drive, darkness, hypnosis, beatShape, and BPM from a
    /// `MusicalIntent`. These are the inputs to `TechnoScene.init(seed:bpm:drive:darkness:hypnosis:beatShape:)`.
    /// Additional render parameters (aggression, atmosphere, chaos) are
    /// communicated through the existing `RenderCharacter` fields and new
    /// derived fields on `TechnoScene` (added in Step 3).
    public static func map(intent: MusicalIntent, seed: UInt64) -> (
        drive: Double,
        darkness: Double,
        hypnosis: Double,
        beatShape: Double,
        ...
        // --- Evolution branch ---
        let paceOfChange = intent[.paceOfChange]
        // Pace of change biases BPM: faster pace → slightly higher tempo
        let bpmBase: Double = 125.0
        let bpm = min(max(bpmBase + paceOfChange * 15.0 + drive * 5.0, TechnoScene.bpmRange.lowerBound), TechnoScene.bpmRange.upperBound)
