...
Quality-contract schema 35, candidate-vector schema 32, candidate-transaction
schema 4, and canonical engine v34 provide the current transaction-level
...
The Professional Evidence v18 bank must contain every canonical checkpoint for
every included rate and complete exact-role masking/stem evidence. The exact
engine-v34 primary evaluator v16 and pinned profile v16/adversarial/holdout
...
by quality-contract schema 35 change detached preparation only. The implemented master-headroom path
