...
and transition-tail continuity to the primary-v25 and long-horizon-v12
qualification sets.
...
semantic trajectory report. Phase 2 embeds one compact renewable arc/episode
...
Phase 6B now binds those dimensions to one exact engine-v44/primary-v25
...
Phase 8 applied the sound-maturation gate to the original Stage 6 evidence and
found no capability-local failure. The later complete AudioReakt audit supplied
separate repeated source evidence plus direct PCM witnesses for two bounded
deficits: session-static kick source identity and fixed clap body. Those were
replaced in place by one kick morphology and contextual clap/snare/rim bodies.
The complete 276-video Mordio audit then selected one bounded granular-memory
texture inside the existing phrase-composition renderer; the regenerated Stage
6 policy exposes no remaining capability-local failure.
...
These refinements constrain operator selection and calibration. Phase 2 records
reserve, recency, obligations, and variable episode geometry; Phase 3 consumes
that context without encoding a fixed set order or adding a second planner.
...
are now **passed** for the exact engine-v44/primary-v25/long-horizon-v12 artifact set. Real
...
`LongHorizonPlanningBaselineProbe` plans the exact fixed-seed runtime for
7,800 bars, or four hours at 130 BPM, twice through the real director and
continuation. Its historical test-only `long-horizon-planning-baseline.v1` report emits
...
Stage 3 deliberately advances that test-only snapshot to
`long-horizon-planning-baseline.v2`. The seed-48,291 run now ends at 7,801 bars
across 714 phrases. Its plan and bar-evidence fingerprints are
`e65f85be9fbf46b6` and `ed54cedc5f2c73c0`; phrase-kind counts are
...
back into planning. The historical Phase 1 four-hour seed-48,291 journey retains 7,800 bars
...
identity landmarks, payoff/recovery/recall obligations, and one bounded causal
material-world lineage with the current and recent completed worlds.

Continuation now carries two explicit clocks. The canonical accepted bar
remains the owner of resolved score, quality evidence, debt, and commit
provenance. A separate bounded presentation bar advances when accepted PCM must
repeat while a successor is still recovering. Episode and material-world aging
consume that presentation clock so recovery does not freeze long-form time, but
repeated PCM does not fabricate accepted phrases, evidence use counts, or score
history. The next committed plan must join both clocks exactly; gaps, overflow,
or root mismatch fail closed without partial mutation.
...
Before selection consumed it, seed 48,291's Phase 2 four-hour continuation completed 31 episodes and
...
episode geometry. A historical test replaced only the long-horizon context
behind identical local phrase memory and observed a distinct context but an
identical next plan. That proved the Stage 2 boundary before Stage 3 crossed it.
...
continuation state only. Stage 3 changes phrase choice and therefore the
resolved score/PCM reached through the existing renderer, but adds no renderer,
DSP, graph, App scheduling, route lifecycle, or realtime-callback mechanism.

### Implemented Phase 3 episode operator selection

`AutonomousSessionDirector` now has one selection path. It validates that the
bound hierarchy matches the exact root, phrase index, and bar boundary; invalid
or discontinuous context uses the previous conservative threshold policy. While
an episode is below its minimum hold, local phrasing continues, but an early
payoff or identity return that would spend the episode's reserved rare event is
held at `lock`. At eligibility, maintain maps to `lock`, rise to `contrast`,
recover to `majorBreak`, payoff to `energyRelease`, recall to
`identityReturn`, and reframe chooses `contrast` or `majorBreak` from the
episode-start role density. A debtless payoff first selects `contrast` to open
canonical dramatic debt, then pays it on the next phrase. Candidate count stays
one.

Each plan carries Codable Core provenance under
`autotechno-long-horizon-selection.v1`: episode ID, operator, selected existing
phrase kind, and one of conservative fallback, minimum hold, reserved payoff,
reserved recall, payoff-debt establishment, or operator fulfillment. Identical
complete context replays the selection and whole plan exactly.

In the seed-48,291 four-hour journey all six operators are fulfilled. Seven
payoffs occur in seven distinct arcs; all seven consume real dramatic debt and
are followed by seven recovery episodes, with a maximum payoff-to-recovery
completion span of 268 bars. Operator fulfillment makes eligible episodes end
on their achieved semantic obligation: the revised four/eight-hour hierarchy
snapshots complete 37/75 episodes through arcs 7/16, with state/sequence
fingerprints `9790c22d98b4807e` / `a3929317ea4e86ac` and
`008b96f48b2e955d` / `ffb454d66004e4bf`.

The unchanged semantic schema truthfully reflects the changed canonical plan:
7,801 bars, 714 phrases, trajectory fingerprint `461af2c64f13f52e`, sorted-
report fingerprint `6b17c975fc72daa8`, 670 descriptive high-tension bars, 390
recovery bars, 130 matched and 553 unmatched identity-return bars, 288 opened
and 284 paid dramatic debts, and 4,273 repeated event-signature observations.
These values prove deterministic causality and coverage, not entertainment or
professional quality. No synthesis architecture, preset, patch, DSP primitive,
...
`candidate-plan.typed.v21` provenance domain fingerprints the selection,
coordination, and per-bar harmonic relationship, so the frozen fallback fixture
now fingerprints to `f46daaca9b68b400`. Detached preparation
...
fingerprints move to `1a8af31815eb50da` / `6132410feb8726e3`, with 202 matched
...
renderer, callback work, or qualified policy. Phase 5 supplies truthful
effect/capability dose and sentence evidence from the current palette;
compatible realized signal trajectories and calibration remain next.

### Implemented Phase 5 effect-dose and sentence evidence

Every canonical phrase now derives at most one
`autotechno-long-horizon-effect-sentence.v1` annotation from an effect relation
already present in its resolved score. A contrast phrase may name its existing
gated percussion answer as a supporting call/response; an energy-release phrase
may name its existing anticipation swell as a foreground turnaround. The
annotation binds phrase, bar, source voice/step, answer window, gesture,
capability, function, and attention priority. It creates no event, permission,
send, return, graph node, or renderer choice. Unsupported or internally
inconsistent decoded records fail closed, and detached preparation re-derives
the exact sentence before rendering.

`AutoTechnoDSP` now exposes
`autotechno-long-horizon-effect-dose.v2` from the accepted prepared phrase. It
reduces existing graph-input/output timbre fingerprints, typed instrument effect
access, pulse-echo return/drive evidence, percussion-return evidence, spatial
FDN evidence, and masking observations into bounded per-bar and per-family
facts. The fixed-capacity session accumulator tracks eligibility, activation,
wet occupancy, tail-only bars, true recovery, active-run length, inactive gaps,
last active bar, return-to-source level, and the last sixteen realized
sentences. It retains hashes and scalars only; no PCM or renderer state survives
the reduction. Invalid causality, discontinuity, root mismatch, or overflow
marks evidence unavailable without partially changing accepted counters.

The seed-48,291 four-hour score scan contains 63 existing sentences: 48 gated
call/responses and 15 anticipation turnarounds. A detached 8 kHz witness binds
one planned sentence to exact source/return fingerprints, positive RMS,
...
exact long-horizon-v12 policy consumes its compatible reduced dimensions only
after primary-qualified preparation.

Phase 5 changes no resolved event, score permission, graph, synth, patch, DSP
primitive, effect implementation, bus, chain, renderer, scheduled PCM, App
scheduler, route lifecycle, or realtime callback. The current palette already
expresses both required sentence functions, so no sound-capability expansion is
justified.

### Implemented Phase 6A realized signal trajectory evidence

`AutoTechnoDSP` now exposes
`autotechno-long-horizon-signal-trajectory.v1` from the same accepted prepared
phrase. It joins the exact typed plan, candidate-evidence, PCM, route-rate,
full-mix, effect-dose, and per-bar identities, then reduces them to named
loudness, true-peak, crest, spectrum, transient, masking, wet/dry, stereo, and
movement dimensions. The semantic energy target remains separate from the
realized vector; this layer cannot call a declared recovery or rise successful.

The fixed-capacity checkpoint accumulator records metric ranges, each
operator's physical transition deltas, bounded episode summaries, and only the
latest 32 phrases/transitions and 16 episodes. Sparse representative renders
record exact omitted phrase and bar counts rather than pretending to be
continuous PCM. Root, sample rate, phrase/bar order, episode re-entry,
inconsistent evidence, and counter arithmetic fail closed without partially
changing accepted summaries. Raw PCM, render blocks, stems, graph state, and
renderer continuation are not retained.

An exact detached 8 kHz phrase witness binds the semantic target to accepted PCM
evidence. A deterministic eight-hour consequence harness covers 1,450 phrases
and 15,603 bars, exercises all six operators, retains fixed capacities, and
fingerprints to `9e39ea3d77b7fb20`. Sparse-checkpoint, semantic/physical
separation, wrong-rate, repeated-phrase, transactional failure, and byte-for-
byte replay cases pass. This is compatible evidence, not calibration: the
report still declares `no-calibrated-long-horizon-policy`.
...
`AutoTechnoDSP` now owns the reduced, versioned Stage 6 calibration chain. Seven
development roots (`48291`, `77777`, `90909`, `112358`, `141421`, `173205`,
`246813`) each contribute an
...
attacks through adversarial artifact `3ff4d0d51c72c99e`: permanent peak, fixed
...
accepted by holdout artifact `d59e4ef6080bbe77`. The bundled artifacts enforce canonical
...
must pass the exact primary-v25 evaluator. Phase 6B changes no score, synth,
...
capabilities and exposes no additional repeatable sound-family deficit.

### Implemented Phase 7 bounded future adaptation

`AutoTechnoDSP` now owns fixed-capacity
`autotechno-long-horizon-runtime-observation.v2` state. It reduces accepted
...
Core owns strict `autotechno-long-horizon-trajectory-decision.v2` provenance.
...
when the projected canonical continuation is eligible; a material/effect-world
deficit can instead request one `reframe-material` action. The v3 continuation
...
Focused local evidence proves exact v3 four/eight-hour replay while retaining
bounded episode and material-world history. A real accepted 8 kHz prepared
...
against every conditional item in the sound-capability register. All seven
...
without creating another director; Stage 3 lets the existing director consume
it:
...
| 6. Calibrated long-horizon policy — complete locally | Primary-v25 evaluator, DSP-owned `autotechno-long-horizon-signal-trajectory.v1`, and immutable Stage 6 artifact loader | Non-compensable cross-phrase/cross-episode semantic, phrase-kind-consistent operator/signal, and effect bounds over seven exact development journeys | Engine-v44 v12 profile, ten rejected attacks, and two fresh disjoint at-least-four-hour holdouts accepted at 44.1/48 kHz | Reduced detached evidence only; exact identities and canonical JSON fail closed; no PCM retained or scheduled audio rewritten | No parallel permissive evaluator, PCM ledger, aggregate engagement score, or semantic claim masquerading as signal consequence |
...
| 8. Targeted sound maturation — complete locally with three bounded source-driven promotions | Existing Core kick, upper-percussion, and phrase-composition score intent; primary-v25 phrase verdict; and affected DSP evidence owners | Complete AudioReakt and Mordio source convergence plus exact PCM exposed session-static kick identity, a fixed clap body, and cut-only slice texture; all other proposed machinery reconciled to existing owners | Multi-rate kick morphology, clap/snare/rim, cut-neutral and deterministic granular-memory witnesses; current seven-journey development, ten-attack adversarial, and two-holdout v11 replay | Score-resolved state-free DSP only; a future promotion still requires repeated independent-root and two-rate causal failure and must replace provisional DSP in place | No preset pack, granular side engine, alternative renderer, speculative chain, genre selector, or duplicate event lane |
| 9. Material-world lineage and effect morph — complete and offline-qualified | Core continuation v4 and `AutonomousPhrasePlan`; existing generated graph and evidence transaction | Causal four-to-eight-minute worlds changing rhythm, motif, roles, harmony, timbral architecture, and one five-coordinate effect target; one-node-per-phrase graph convergence | 512-root property sweep, different ten-/twenty-minute snapshots, exact replay and route recovery, primary-v25 qualification, and long-horizon-v12 development/adversarial/fresh-holdout pass | New worlds and reframe-material actions apply only at eligible future boundaries; malformed input preserves the accepted world; graph rejection freezes topology | No random graph-operation roulette, preset bank, selectable FX chain, alternative renderer, or second director |
