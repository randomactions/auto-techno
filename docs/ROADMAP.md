...
Auto Techno has one executable, one current runtime, one canonical score, and one
continuing performance identity. Every increment must strengthen that path and
leave behind a reusable engine capability. The roadmap records durable outcomes,
not temporary test counts, toolchain snapshots, or pending listening verdicts.
...
- one bounded terminal hang on the final kick-withheld energy-release bar that
  preserves the existing weak-pulse setup through step 11, applies one 8 ms
  output release, holds steps 12--16 at exact silence, and leaves the unchanged
  next-bar recovery and continuing DSP state authoritative;
- one score-owned percussion-return texture that reuses an existing early
...
- one score-owned three-sixteenth pad-modulation relation that restores rhythmic
  energy in the latter half of eligible major breaks by reusing the existing pad
  low-pass and spatial send, with no new note lane, sequencer, or effect bus;
- one bounded two-bar dotted foundation relation that replaces only existing
  bass onsets in eligible Lock pairs, reuses the protected Bass Pluck path, and
  restores the phrase boundary without another track, clock, or effect return;
...
- one score-owned protagonist spectral-reveal relation that reuses only an
  existing Resonant Mono or Tonal Motion anchor cutoff during a naturally
  emerging lock or contrast, with exact home correction and no new lane;
...
- one score-owned modal-percussion foundation voice replacing the root-only
  realization with four material-world-selected coupled bodies, eight stable
  modes, four fixed continuation slots, protected routing, and exact same-pass
  evidence;
- one score-derived pre-kick terminal release on the eligible dotted Bass
  Pluck, with route-derived geometry, an exact-zero protected-foundation pocket,
  and non-compensable contamination evidence;
...
- canonical engine v48, quality-contract schema 49, candidate-vector schema 43,
  and candidate-transaction schema 14, with the explicit
...
- phrase-wide and live-window ITU-R BS.1770-5 loudness, Annex 2 true peak,
  ...
  Professional Evidence v29 report bank;
...
The app installs one exact-engine calibrated primary evaluator v29 from the
current profile v29 derived from 36 complete journeys, a passing v22 adversarial
suite, and a disjoint four-journey holdout qualification v20. Each boundary owns one
...
over growing parallel rule systems.

## Ordered outcomes

### 1. Close the existing control path

Inventory semantic, scene, modulation, and render state against actual PCM and
telemetry consumers. Connect state that belongs in the canonical mechanism;
remove state that has no audible or evaluative responsibility. Establish a test
that prevents disconnected parameters from appearing integrated.

### 2. Establish versioned quality evidence

Quality-contract schema 49, candidate-vector schema 43, and candidate-
transaction schema 14 now bind a versioned transaction across canonical engine
...
and exact arpeggiator score-to-render pitch binding, a score-owned pad
filter/send/amplitude rhythm with exact closed-step silence, plus exact paid-debt-to-release climax
...
spectral, masking, and pole-stability evidence. Professional Evidence v29 adds
...
guardrails, a passing 34-case v19 adversarial suite, and a disjoint holdout report
...
and controller state into continuation. The exact engine-v48 profile v29,
passing v22 adversarial suite, disjoint holdout qualification v20, non-compensable
...
Iteratively improve oscillators, filters, distortion, dynamics, space, role
...
primitive without adding a track, instrument, effect bus, master chain, or
control, and retains exact pre/post transient and spectral evidence. A later
...
Exercise complete journeys across structural checkpoints and extended playback.
Require automated evidence for artifact freedom, translation, controller
stability, identity, groove, and long-form coherence, plus sufficient preparation
...
report. Phase 2 now carries `autotechno-long-horizon-continuation.v6` in
...
now binds the exact engine-v48/primary-v29 policy to seven development journeys,
...
- adding user-facing treatment, mastering, rhythm, synth-engine, seed, or
  performance-model switches;
- treating a VSTi, Audio Unit, DAW, cloud service, or reference recording as a
  runtime dependency;
- using one opaque score as a substitute for hard constraints and reason-coded
  quality evidence;
- adjusting already scheduled audio or moving planning, analysis, or mutable
  preparation onto the real-time callback;
- requiring a human listening verdict to promote a musical implementation.
