...
| A climax should pay an earlier dramatic obligation rather than appear as an unrelated loud section. | The existing director opens bounded contrast or major-break debt, then a nonconservative energy release resolves it; when the existing kick-syntax relation is present, that payoff also owns the grounded setup, two-bar kick withholding, and structural recovery. Source 20 adds no new PCM; it makes that long-form cause explicit. | Candidate schema 15 fingerprints the exact paid debt records, retains contrast/major-break counts and temporal bounds, cross-checks the optional four-bar kick consequence, and keeps conservative or debt-free phrases inactive. Shipping selection remains uncalibrated. | Replace the current coarse phrase-kind/debt and optional fixed four-bar recovery realization with a calibrated multi-stage energy trajectory, higher-resolution transition envelopes, and role-aware payoff orchestration only after long-form evidence exposes a repeatable deficit. Preserve debt ownership, exact future-boundary application, fallback, and causality; do not add a second break sequencer or off-grid transport. **Revisit when:** a calibrated paired journey rejects a structurally valid debt-to-release phrase for deficient tension/payoff trajectory without safety or identity regression. **Supersedes:** the v16 compact climax-arc projection and current kick-syntax realization in place rather than coexisting with another climax mode. |
